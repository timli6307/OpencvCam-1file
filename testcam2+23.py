from collections import  deque
import numpy as np
import cv2
import serial
from time import sleep
import sys

# COM_PORT = 'COM14'  # 請自行修改序列埠名稱
# BAUD_RATES = 9600

# ser = serial.Serial(COM_PORT, BAUD_RATES)


cap1 = cv2.VideoCapture(0,cv2.CAP_DSHOW)
cap = cv2.VideoCapture(1,cv2.CAP_DSHOW)

cap1.set(cv2.CAP_PROP_FRAME_WIDTH, 500)
cap1.set(cv2.CAP_PROP_FRAME_HEIGHT, 500)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 500)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 500)

# mybuffer = 16
# pts = deque(maxlen=mybuffer)

redLower = np.array([0, 43, 46])
redUpper = np.array([10, 255, 255])

blueLower = np.array([100, 43, 46])
blueUpper = np.array([124, 255, 179])

greenLower = np.array([35, 43, 46])#綠
greenUpper = np.array([77, 255, 255])

purpleLower = np.array([122, 43, 46])#紫
purpleUpper = np.array([155, 255, 255])

q = None


while(True):
    ret,frame = cap.read()
    ret1,frame1 = cap1.read()
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv1 = cv2.cvtColor(frame1, cv2.COLOR_BGR2HSV)

    maskR = cv2.inRange(hsv, redLower, redUpper)#夾子
    maskR = cv2.erode(maskR, None, iterations=2)
    maskR = cv2.dilate(maskR, None, iterations=2)
    maskB = cv2.inRange(hsv, purpleLower, purpleUpper)#目標物
    maskB = cv2.erode(maskB, None, iterations=2)
    maskB = cv2.dilate(maskB, None, iterations=2)
    maskR1 = cv2.inRange(hsv1, redLower, redUpper)#夾子
    maskR1 = cv2.erode(maskR1, None, iterations=2)
    maskR1 = cv2.dilate(maskR1, None, iterations=2)
    maskB1 = cv2.inRange(hsv1, purpleLower, purpleUpper)#目標物
    maskB1 = cv2.erode(maskB1, None, iterations=2)
    maskB1 = cv2.dilate(maskB1, None, iterations=2)

    cnts1 = cv2.findContours(maskR.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    cnts2 = cv2.findContours(maskB.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    cnts11 = cv2.findContours(maskR1.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    cnts21 = cv2.findContours(maskB1.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    
    center1 = None
    center2 = None
    center3 = None
    reset = (200,440)
    cv2.circle(frame, reset, 10, (0, 0, 255), -1)
    cv2.circle(frame1, reset, 10, (0, 0, 255), -1)

    if len(cnts1 or cnts2) != 0:
        
        c1 = max(cnts1, key = cv2.contourArea)#找到面積最大的輪廓
        c2 = min(cnts2, key = cv2.contourArea)#找到面積最小的輪廓

        M1 = cv2.moments(c1)#計算輪廓的矩
        M2 = cv2.moments(c2)#計算輪廓的矩

        center1 = (int(M1["m10"]/M1["m00"]), int(M1["m01"]/M1["m00"]))#計算質心
        center2 = (int(M2["m10"]/M2["m00"]), int(M2["m01"]/M2["m00"]))#計算質心
        
        pointAx = int(M1["m10"]/M1["m00"])#(Ax,Ay)Redpoint
        pointAy = int(M1["m01"]/M1["m00"])
        pointBx = int(M2["m10"]/M2["m00"])#(Bx,By)Bluepoint
        pointBy = int(M2["m01"]/M2["m00"])
        pointCx = 500
        pointCy = 300
        
        cv2.circle(frame, center1, 10, (0, 0, 255), -1)#Redpoint
        cv2.circle(frame, center2, 10, (255, 0, 0), -1)#Bluepoint


        cv2.line(frame,reset,center1,(0, 0, 255),1,4)
        cv2.line(frame,reset,center2,(0, 0, 255),1,4)
        
        # m1=(pointAy-pointCy)/(pointAx-pointCx)#RC斜率
        # m2=(pointBy-pointCy)/(pointBx-pointCx)#BC斜率
        # print(m1,m2)
        t:float=0.05
        # ser.write(m1.encode('utf-8'),m2.encode('utf-8'))






    hsv2 = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        
    cv2.imshow('dectect point',frame)
    cv2.imshow('dectect point',frame)
    if cv2.waitKey(1) & 0xFF ==ord('q'):
        break
    
    k = cv2.waitKey(1)&0xFF
    if k == 27:
        break


# ser.close()
cap.release()
cap1.release()
cv2.destroyAllWindows()
sys.exit()