...
while(True):
    ...
    mask2R = cv2.inRange(side_hsv, redLower, redUpper)
    mask2R = cv2.erode(mask2R, None, iterations=2)
    mask2R = cv2.dilate(mask2R, None, iterations=2)
    mask2Y = cv2.inRange(side_hsv, purpleLower, purpleUpper)
    mask2Y = cv2.erode(mask2Y, None, iterations=2)
    mask2Y = cv2.dilate(mask2Y, None, iterations=2)
    ...
    cnts_side1 = cv2.findContours(mask2R.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    cnts_side2 = cv2.findContours(mask2Y.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    ...
    if len(cnts_up1 and cnts_up2 and cnts_side1 and cnts_side2) != 0:
        ...
        pointDy = int(SIDE2["m01"]/SIDE2["m00"])
        ...
        strw = str(pointAx)+','+str(pointAy)+','+str(pointBx)+','+str(pointBy)+','+str(pointCx)+','+str(pointCy)+','+str(pointDx)+','+str(pointDy)
        # ser.write(strw.encode('utf-8'))
        m1=(pointAy-480)/(pointAx-900)
        m2=(pointBy-480)/(pointBx-900)
        
        print(m1,m2)
        # ser.write(pointAx.encode('utf-8'),pointAy.encode('utf-8'),
        #           pointBx.encode('utf-8'),pointBy.encode('utf-8'),
        #           pointCx.encode('utf-8'),pointDy.encode('utf-8'),
        #           pointCx.encode('utf-8'),pointDy.encode('utf-8'))
    ...
    frameX = np.hstack((up_frame,side_frame))
    cv2.imshow('ALLframe',frameX)
